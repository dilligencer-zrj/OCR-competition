...
class Model(object):
    ...
    def __init__(self,
        ...
        # variables
        self.img_data = tf.placeholder(tf.float32, shape=(None, 3, 48, None), name='img_data')
    ...
    def launch(self):
        ...
        if self.phase == 'test':
            ...
            for batch in self.s_gen.gen(self.batch_size):
                ...
                img_data = np.transpose(img_data, axes=[0, 3, 1, 2])
    ...
    def visualize_attention(self, filename, attentions, output_valid, ground_valid, flag_incorrect, real_len):
        ...
        with open(os.path.join(output_dir, 'word.txt'), 'w') as fword:
            fword.write(' '.join([vocabulary[c-3]  for c in ground_valid])+'\n')
            fword.write(' '.join([vocabulary[c-3] for c in output_valid]))
