...
class BucketData(object):
    ...
    def append(self, datum, label, filename):
        ...
        self.data_len_list.append(int(math.floor(datum.shape[-2] / 4)))
        self.label_list.append(label)
        self.file_list.append(filename)

        self.max_width = max(datum.shape[-2], self.max_width)
    ...
    def flush_out(self, bucket_specs, valid_target_length=float('inf'),
        ...
        def get_bucket_id():
            for idx in range(0, len(bucket_specs)):
                if bucket_specs[idx][0] >= self.max_width / 4  \
        ...
        real_len = max(int(math.floor(self.max_width / 4)), 0)
        padd_len = max((int(encoder_input_len) - real_len), 0)
        print(real_len,padd_len)
